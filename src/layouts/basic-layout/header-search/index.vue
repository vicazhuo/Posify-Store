<template>
  <span v-if="isDesktop"
    ...
    <Select v-model="currentVal" placeholder="搜索..." filterable remote :remote-method="remoteMethod" :loading="loading">
      <Option v-for="(option, index) in menusList" :value="option.menu_path" :key="index" :disabled="option.type === 1">
    ...
    <Dropdown trigger="click" class="i-layout-header-search-drop" ref="dropdown">
      ...
      <DropdownMenu slot="list">
        <div class="i-layout-header-search-drop-main">
          ...
          <Select v-model="currentVal" placeholder="搜索..." filterable size="large" remote :remote-method="remoteMethod"
            :loading="loading">
            <Option v-for="(option, index) in menusList" :value="option.menu_path" :key="index"
              :disabled="option.type === 1">{{ option.menu_name }}</Option>
          </Select>
          <span class="i-layout-header-search-drop-main-cancel" @click="handleCloseSearch">{{
