    ...
    <div style="width: 100%">
        <Modal v-model="modals" scrollable footer-hide closable title="用户充值" :mask-closable="false" width="583" @on-cancel="cancel">
            <div slot="header" class="header">
				<div class="header_tab left" :class="tabs==1?'on':''" @click="tab(1)">优惠充值</div>
				<div class="header_tab" :class="tabs==2?'on':''" @click="tab(2)">自定义充值</div>
			</div>
			<div class="content" v-if="tabs == 1">
				<div v-for="(item,index) in data" class="content_box" :class="contTabs == index?'content_tab':''" @click="contTab(item,index)">
					...
					<div class="bottom">额外赠送：¥ {{item.give_money}}</div>
				</div>
			</div>
			<div class="contents" v-else>
				<div>
					 <Input v-model="value" placeholder="0.00" style="width: 100%" type="number"/>
					 <div class="title">自定义充值的金额无赠送优惠</div>
				</div>
			</div>
			<!-- <div class="footer" @click="save">确定</div> -->
			<div class="footer">			
				<Button style="margin-right:14px" type="default" @click="close" >取消</Button>
				<Button type="primary" v-preventReClick @click="save" >确定</Button>
			</div>
